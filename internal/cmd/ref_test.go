...
func TestRunRefReplace_UnscannableDashboardRefusesUnlessAllowPartial(t *testing.T) {
	...
	withRefConfirm(t, true)
	var buf bytes.Buffer
	err := refReplaceWithOptions(context.Background(), &buf, "sensor.old", "sensor.new", flagRefConfirm, flagRefAllowPartial)
...
// TestRunRefScan_UnscannableDashboardStillAnswers is D-7 point 5, as amended by
// WP7: a search tool answers "where is X?", so a dashboard it could not read
// never makes the command fail in plain text — but the scope now goes into the
// REPORT BODY rather than only to slog.
//
// This test used to assert the opposite half in its own words: "the skip is a
// slog.Warn on stderr, never a row or a prose line on stdout", with the stdout
// shape pinned as the contract. That was the design at the time and it is what
// #34 walked through — a warning is invisible to a machine and to anyone whose
// HACTL_LOG_LEVEL hides it, and the same code path dropped the WHOLE config half
// (21 of 24 references on the reference instance) through the same door. The
// stdout shape is a contract under --json, where it is now defended by refusing
// (TestRunRefScan_PartialRefusesUnderJSONUnlessAllowPartial); in plain text a
// line above the table is exactly where a person can see it.
func TestRunRefScan_UnscannableDashboardStillAnswers(t *testing.T) {
	...
	// The scope reaches the answer, and it names the dashboard and the reason —
	// "0 of 1 dashboard(s) scanned" is the fact, "boom" is why.
	for _, want := range []string{"partial sweep", "dashboard(s) scanned", "boom"} {
		if !strings.Contains(out, want) {
			t.Errorf("the scope must reach the report body, missing %q\n%s", want, out)
		}
	}
}
