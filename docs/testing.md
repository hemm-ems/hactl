...
**`domaindecode/`** is the oracle fixture for `/api/states` attribute typing. `auto ls` and `script ls` decode *every* entity in the instance into their own domain-typed struct and filter to `automation.`/`script.` afterwards, so an entity they will discard can fail them — which is what a live instance reported (`cannot unmarshal number -1.7525 into … automationAttributes.attributes.current of type int`). That instance is a third party's and unreachable, so the fixture does not try to reproduce its entity: it enables `demo:` for breadth (127 entities across 39 domains) and adds `sensor.hactl_synthetic_collider`, a `template:` sensor named so it cannot be mistaken for an observation, carrying every key those structs decode at a JSON type the Go field cannot hold. `domaindecode_oracle_test.go` asks HA the three questions the fix rests on — whether one attribute key can carry two JSON types in one response (it can, and HA's own `max` does it: an integer on automations, a fraction on `number.*`), whether `current` is integral by construction (it is — HA computes it as `len(self._runs)`, read out of the running container), and which of the six keys can collide (all of them are reachable, so the acceptance test covers all six). The fixture also reproduces the reported error verbatim, which is what makes it a reproduction rig and not just a probe.
